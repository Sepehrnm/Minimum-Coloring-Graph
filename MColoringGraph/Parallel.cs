...
namespace MColoringGraph
{
    public class ParallelGraphColoring
    {
        ...
        public ParallelGraphColoring(int v)
        {
            V = v;
            colors = new int[V];
        ...
        private bool IsSafe(int v, int c)
        {
            ...
            // Using Parallel.ForEach to iterate over adjacent vertices concurrently
            Parallel.ForEach(adjList[v], adjacent =>
            {
                if (colors[adjacent] == c)
                {
                    isSafe = false; // If a conflict is found, mark it as unsafe
